...
export default function DashboardPage() {
  ...
  // --- 2. ユーティリティ・計算ロジック（エラー ts(2304) の根絶） ---
  ...
  const calculateTotalBreak = (brks: any[], start: string | null = null) => {
  ...
  const calculateWorkMins = (cIn: string, cOut: string | null, brks: any[]) => {
    ...
    return Math.max(0, diff - calculateTotalBreak(brks));
  };
  ...
  const fetchPersonalHistory = useCallback(async (staffId: string) => {
    ...
    const { data } = await supabase.from('timecards').select('*, breaks(*)').eq('staff_id', staffId).gte('work_date', start).order('work_date', { ascending: false });
    if (data) setPersonalHistory(data.map((r: any) => ({ ...r, work_time: formatHHMM(calculateWorkMins(r.clock_in_at, r.clock_out_at, r.breaks)), break_time: formatHHMM(calculateTotalBreak(r.breaks)) })));
  }, [currentTime]);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (s) {
        ...
        if (s.role === 'admin') {
          const { data: l } = await supabase.from('staff').select('id, name'); if (l) setAdminStaffList(l);
          fetchAdminMonitor(monitorDate);
        }
    ...
    return () => { clearInterval(timer); window.removeEventListener('resize', resizeHandler); };
  }, [syncStatus]);

  // --- 4. 業務ロジック（前後30分 ＆ リカバリー） ---
  ...
  const handleClockAction = async (type: 'in' | 'out' | 'break') => {
    ...
    if (type === 'out') { if(!confirm("退勤を記録しますか？")) return; await supabase.from('timecards').update({ clock_out_at: new Date().toISOString() }).eq('staff_id', staff.id).is('clock_out_at', null); }
    if (type === 'break') {
  ...
  const handleTaskComplete = async (e: any) => {
    ...
    try {
      ...
      setActiveTask(null); setIsQrVerified(false); fetchTasks(); alert("完了報告を送信しました。");
    } catch { alert("送信エラー"); }
  ...
  const downloadCSV = () => {
    ...
    const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `BE_STONE_Report.csv`; link.click();
  };
  ...
  const currentBreakObj = breaksList.find(b => !b.break_end_at);
  const tValue = attendanceStatus === 'break' ? `${Math.floor((currentTime.getTime() - new Date(currentBreakObj?.break_start_at).getTime())/60000)}分 (計:${calculateTotalBreak(breaksList, currentBreakObj?.break_start_at)}分)` : formatHHMM(calculateWorkMins(currCard?.clock_in_at, null, breaksList));
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row text-black overflow-x-hidden font-sans">
      ...
      <aside className={`fixed md:relative inset-y-0 left-0 z-[150] w-[75vw] md:w-72 bg-white border-r p-6 flex flex-col transition-transform ${sidebarOpen || !isMobile ? 'translate-x-0' : '-translate-x-full'}`}>
        <h1 className="text-2xl font-black italic text-[#75C9D7] mb-8">BE STONE Pro</h1>
        ...
        <div className="max-w-4xl mx-auto">
              ...
              <div className="space-y-6 animate-in fade-in">
                <div className="app-card border-l-8 border-[#75C9D7]">
                  {attendanceStatus === 'offline' ? (
                    <button onClick={() => handleClockAction('in')} className="w-full py-5 bg-[#75C9D7] text-white font-black rounded-2xl text-xl border-none">🚀 業務開始 (出勤)</button>
                  ) : (
                    <div className="text-center">
                      <div className="flex gap-3 mb-4">
                        <button onClick={() => handleClockAction('break')} className={`flex-1 py-4 border-none ${attendanceStatus === 'break' ? 'bg-orange-400' : 'bg-[#1a202c]'} text-white font-black rounded-2xl`}>{attendanceStatus === 'break' ? '🏃 業務復帰' : '☕ 休憩入り'}</button>
                        <button onClick={() => handleClockAction('out')} className="flex-1 py-4 bg-white border border-slate-200 text-slate-400 font-black rounded-2xl">退勤</button>
                      </div>
                ...
                {attendanceStatus !== 'offline' && displayTasks.map(t => (
                  <div key={t.id} className="app-card flex justify-between items-center border-l-8 border-[#75C9D7]">
                    <div className="flex-1 pr-4 text-black">
                      ...
                      <h5 className="text-lg font-bold">{t.task_master?.task_name}</h5>
            ...
            {menuChoice === "⚠️ 未完了タスク" && overdueTasks.map(t => (
              <div key={t.id} className="app-card border-l-8 border-red-400 flex justify-between items-center text-black">
                <div className="flex-1">
                  <p className="text-red-500 font-black text-xs">【遅延】{t.task_master?.target_hour}:00</p>
                  <h5 className="text-lg font-bold">{t.task_master?.task_name}</h5>
            ...
            {menuChoice === "🕒 自分の履歴" && personalHistory.map(r => (
              <div key={r.id} className="app-card border-l-8 border-[#75C9D7] text-black">
                <p className="font-black text-sm">{r.work_date} ({formatToJSTTime(r.clock_in_at)}〜{formatToJSTTime(r.clock_out_at)})</p>
                <p className="text-xs font-bold text-slate-600 mt-1 text-black">実働: {r.work_time} / 休憩: {r.break_time}</p>
              </div>
            ))}

            {menuChoice === "📊 監視(Admin)" && (
              <div className="space-y-6">
                <input type="date" className="p-4 bg-slate-50 rounded-xl font-bold border-none w-full text-center" value={monitorDate} onChange={e => {setMonitorDate(e.target.value); fetchAdminMonitor(e.target.value);}} />
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {adminTasks.map(t => (
                    <div key={t.id} className="app-card p-3 text-center border-b-4 border-[#75C9D7]">
                      ...
                      <div className="text-xs font-black text-slate-800 mb-1">{t.task_master?.locations?.name}</div>
              ...
              <div className="space-y-6">
                <div className="app-card">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
                    <select className="p-3 bg-slate-50 rounded-xl font-bold border-none text-sm" value={filterStaffId} onChange={e => setFilterStaffId(e.target.value)}><option value="all">全員</option>{adminStaffList.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select>
                    <input type="date" className="p-3 bg-slate-50 rounded-xl border-none" value={filterStartDate} onChange={e => setFilterStartDate(e.target.value)} />
                    <input type="date" className="p-3 bg-slate-50 rounded-xl border-none" value={filterEndDate} onChange={e => setFilterEndDate(e.target.value)} />
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <button onClick={generateAdminReport} className="admin-grid-btn bg-[#1a202c]"><Search size={16}/>抽出</button>
                    <button onClick={downloadCSV} className="admin-grid-btn bg-[#75C9D7]"><Download size={16}/>CSV</button>
                    <button onClick={() => {setEditingCard(null); setEditForm({staff_id: adminStaffList[0]?.id || "", work_date: new Date().toISOString().split('T')[0], clock_in_time: "09:00", clock_out_time: "18:00"}); setIsEditModalOpen(true);}} className="admin-grid-btn bg-orange-400"><Plus size={16}/>追加</button>
                  </div>
                </div>
                {adminReport.map(r => (
                  <div key={r.id} className="app-card flex justify-between items-center border-l-8 border-slate-100 py-4 text-black">
                    <div className="flex-1 text-black"><p className="font-black text-sm">{r.staff_name} 様</p><p className="text-[10px] text-slate-500">{r.work_date} ({formatToJSTTime(r.clock_in_at)}〜{formatToJSTTime(r.clock_out_at)})</p></div>
                    <div className="flex gap-1">
                      ...
                      <button onClick={async () => { if(confirm("削除？")){ await supabase.from('breaks').delete().eq('timecard_id', r.id); await supabase.from('timecards').delete().eq('id', r.id); generateAdminReport(); }}} className="p-2.5 bg-red-50 text-red-400 rounded-lg border-none"><Trash2 size={14}/></button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTask && (
        <div className="fixed inset-0 bg-white z-[300] flex flex-col p-6 pt-10 overflow-y-auto text-center">
