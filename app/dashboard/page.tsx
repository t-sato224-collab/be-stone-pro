...
export default function DashboardPage() {
  ...
  const handleSaveRecord = async () => {
    setLoading(true);
    // 【最重要修正】toISOStringを一切使わず、文字列でJST(+09:00)を強制刻印
    ...
    if (editingCard) {
      await supabase.from('timecards').update({ clock_in_at: cInStr, clock_out_at: cOutStr, work_date: editForm.work_date }).eq('id', cid);
    } else {
      const res = await supabase.from('timecards').insert({ staff_id: editForm.staff_id, staff_name: adminStaffList.find(x => x.id === editForm.staff_id)?.name, clock_in_at: cInStr, clock_out_at: cOutStr, work_date: editForm.work_date }).select();
      cid = res.data?.[0]?.id;
    }
    if (cid) {
      ...
      if (parseInt(editForm.break_mins) > 0) {
        ...
        await supabase.from('breaks').insert({ staff_id: editingCard?.staff_id || editForm.staff_id, timecard_id: cid, break_start_at: cInStr, break_end_at: bE, work_date: editForm.work_date });
      }
    }
  ...
  const timerLabel = attendanceStatus === 'break' ? "休憩中" : "実働中";
  const timerVal = attendanceStatus === 'break' ? `${Math.round((currentTime.getTime() - new Date(breaksList.find(b=>!b.break_end_at)?.break_start_at).getTime())/60000)}分 (計:${calculateTotalBreak(breaksList, true)}分)` : formatHHMM(calculateWorkMins(currCard?.clock_in_at, null, breaksList));
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row text-black font-sans overflow-x-hidden">
      <style jsx global>{`
        ...
        .menu-item { width: 100%; text-align: left; padding: 20px; border-radius: 1rem; font-weight: 900; font-size: 20px; white-space: nowrap; border-bottom: 1px solid #EDF2F7; background: transparent; color: #000000 !important; }
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="app-card border-l-8 border-[#75C9D7] text-center">
                {attendanceStatus === 'offline' ? <button onClick={() => handleClockAction('in')} className="w-full py-5 bg-[#75C9D7] text-white font-black rounded-2xl text-xl border-none shadow-lg">🚀 業務開始 (出勤)</button> : <>
                    ...
                    <p className="text-lg font-black mt-1 text-center" style={{color: attendanceStatus === 'break' ? '#ED8936' : '#75C9D7'}}>{timerLabel}：{timerVal}</p>
                </>}
              </div>
          ...
          {menuChoice === "⚠️ 未完了タスク" && overdueTasks.map(t => (
            <div key={t.id} className="app-card border-l-8 border-red-400 flex justify-between items-center text-black">
              <div className="flex-1 pr-4"><p className="text-red-500 font-black text-xs uppercase mb-1">【遅延】{String(t.task_master?.target_hour).padStart(2,'0')}:00</p><h5 className="text-lg font-bold text-black">{t.task_master?.task_name}</h5></div>
          ...
          {menuChoice === "📅 出勤簿(Admin)" && (
            <div className="space-y-6 text-black">
              ...
              {adminReport.map(r => (<div key={r.id} className="app-card flex justify-between items-center border-l-8 border-slate-100 text-black py-4"><div className="flex-1 text-black"><p className="font-black text-sm text-black">{r.staff_name}</p><p className="text-[10px] text-slate-500">{r.work_date} (休憩:{r.break_time})</p><p className="text-xs font-bold text-[#75C9D7]">実働:{r.work_time}</p></div><div className="flex gap-1"><button onClick={() => handleEditClick(r)} className="p-2.5 bg-slate-50 text-slate-500 rounded-lg border-none cursor-pointer"><Edit size={14}/></button>
        ...
        <div className="fixed inset-0 bg-black/60 z-[400] flex items-center justify-center p-4 backdrop-blur-sm text-black">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl text-black">
            ...
            <div className="space-y-4">
              ...
              <div><label className="text-[10px] font-black text-slate-400 ml-1 text-black">日付</label><input type="date" className="w-full p-3 bg-slate-50 rounded-xl border-none text-sm font-bold" value={editForm.work_date} onChange={e => setEditForm({...editForm, work_date: e.target.value})} /></div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-[400] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl text-black text-center">
            <AlertTriangle size={48} className="text-red-500 mb-4 mx-auto" /><h3 className="text-xl font-black mb-6 text-red-600">削除確認</h3>
