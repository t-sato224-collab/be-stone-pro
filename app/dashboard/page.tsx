...
export default function DashboardPage() {
  ...
  const handleSaveRecord = async () => {
    setLoading(true);
    // 【究極修正】日付オブジェクトを介さず、文字列でJSTを直書き。Supabase側のUTC変換を物理的に回避。
  ...
  const tInfo = attendanceStatus === 'break' ? { label: "休憩中", val: `${Math.round((currentTime.getTime() - new Date(cbObj?.break_start_at).getTime())/60000)}分 (計:${calculateTotalBreak(breaksList, true)}分)`, col: "#ED8936" } : { label: "実働中", val: formatHHMM(calculateWorkMins(currCard?.clock_in_at, null, breaksList)), col: "#75C9D7" };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row text-black font-sans overflow-x-hidden">
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto pt-24 md:pt-10 text-black text-black">
        <div className="max-w-4xl mx-auto">
          ...
          {menuChoice === "⚠️ 未完了タスク" && overdueTasks.map(t => (
            <div key={t.id} className="app-card border-l-8 border-red-400 flex justify-between items-center">
              <div className="flex-1 pr-4 text-black text-black text-black"><p className="text-red-500 font-black text-xs uppercase mb-1">【遅延】{String(t.task_master?.target_hour).padStart(2,'0')}:00</p><h5 className="text-lg font-bold">{t.task_master?.task_name}</h5></div>
          ...
          {menuChoice === "🕒 自分の履歴" && personalHistory.map(r => (
            <div key={r.id} className="app-card border-l-8 border-[#75C9D7] py-4 text-black"><p className="font-black text-sm">{r.work_date} ({formatToJSTTime(r.clock_in_at)}〜{formatToJSTTime(r.clock_out_at)})</p><p className="text-xs font-bold text-slate-600 mt-1 text-black">実働: {r.work_time} / 休憩: {r.break_time}</p></div>
          ))}
          ...
          {menuChoice === "📅 出勤簿(Admin)" && (
            <div className="space-y-6 text-black text-black">
              <div className="app-card text-black text-black text-black text-black">
              ...
              {adminReport.map(r => (<div key={r.id} className="app-card flex justify-between items-center border-l-8 border-slate-100 py-4 text-black"><div className="flex-1"><p className="font-black text-sm text-black text-black">{r.staff_name}</p><p className="text-[10px] text-slate-500">{r.work_date} (休憩:{r.break_time})</p><p className="text-xs font-bold text-[#75C9D7]">実働:{r.work_time}</p></div><div className="flex gap-1">
      ...
      {activeTask && (
        <div className="fixed inset-0 bg-white z-[300] flex flex-col p-6 pt-10 overflow-y-auto text-center text-black">
          <div className="flex justify-between items-center mb-8 px-2 text-black text-black text-black text-black text-black"><button onClick={() => setActiveTask(null)} className="p-3 bg-slate-100 rounded-xl border-none active:scale-90 transition-all"><PauseCircle size={20}/></button><h2 className="text-lg font-black italic">MISSION</h2><div className="w-10"></div></div>
          {!isQrVerified ? <QrScanner onScanSuccess={onQrScan} /> : <div className="space-y-8 animate-in zoom-in duration-300 text-black"><CheckCircle2 size={64} className="text-green-500 mx-auto" /><p className="font-black text-xl">{activeTask.task_master?.task_name}</p><p className="text-xs font-bold text-slate-400 text-center">担当：{staff.name}</p><label className="block w-full px-4 cursor-pointer text-black"><div className="w-full py-8 bg-[#75C9D7] text-white font-black rounded-3xl shadow-xl flex items-center justify-center gap-3 text-xl active:scale-95 transition-all text-white"><Camera size={32}/>完了撮影</div><input type="file" accept="image/*" capture="environment" className="hidden" onChange={handleTaskComplete} /></label></div>}
        </div>
      )}
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-[400] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl text-black">
            <h3 className="text-lg font-black mb-6 text-center text-black text-black">勤怠修正</h3>
            ...
            <div className="flex gap-3 mt-8 text-black text-black">
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-[400] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl text-black">
            <div className="text-center text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black"><AlertTriangle size={48} className="text-red-500 mb-4 mx-auto" /><h3 className="text-xl font-black mb-6 text-red-600">削除確認</h3></div>
