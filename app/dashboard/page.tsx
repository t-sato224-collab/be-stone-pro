...
export default function DashboardPage() {
  ...
  const downloadCSV = () => {
    ...
    const r = adminReport.map(x => `${x.staff_name},${x.work_date},${formatJST(x.clock_in_at)},${formatJST(x.clock_out_at)},${x.break_time},${x.work_time}`).join("\n");
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row text-black overflow-x-hidden">
      <style jsx global>{`
      ...
      <aside className={`fixed md:relative inset-y-0 left-0 z-[150] w-[75vw] md:w-72 bg-white border-r p-6 flex flex-col transition-transform ${sidebarOpen || !isMobile ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-auto pt-4 border-t text-center">
            <p className="font-black text-sm mb-3 text-black">{staff.name}</p>
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-6 animate-in fade-in">
              ...
              {attendanceStatus !== 'offline' && displayTasks.map(t => (
                <div key={t.id} className="app-card flex justify-between items-center border-l-8 border-[#75C9D7] text-black">
                  <div className="flex-1 pr-4"><p className="text-[10px] font-black uppercase text-[#75C9D7]">{t.task_master?.locations?.name}</p><h5 className="text-lg font-bold text-black">【{String(t.task_master?.target_hour).padStart(2,'0')}:{String(t.task_master?.target_minute || 0).padStart(2,'0')}】{t.task_master?.task_name}</h5>{t.status === 'started' && <p className="text-xs text-orange-500 font-bold mt-1">● 進行中</p>}</div>
          ...
          {menuChoice === "⚠️ 未完了タスク" && overdueTasks.map(t => (
            <div key={t.id} className="app-card border-l-8 border-red-400 flex justify-between items-center text-black">
              <div className="flex-1 text-black"><p className="text-red-500 font-black text-xs uppercase mb-1">【遅延】{String(t.task_master?.target_hour).padStart(2,'0')}:00</p><h5 className="text-lg font-bold text-black">{t.task_master?.task_name}</h5><p className="text-xs text-slate-400">{t.task_master?.locations?.name}</p></div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {adminTasks.map(t => (
                  <div key={t.id} className="app-card p-3 text-center border-b-4 border-[#75C9D7] text-black">
                    ...
                    <div className="bg-slate-50 py-1.5 rounded-lg text-[10px] font-black mt-2 text-black"><UserCheck size={10} className="inline mr-1" />{t.staff?.name}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {menuChoice === "📅 出勤簿(Admin)" && (
            <div className="space-y-6 text-black">
              ...
              {adminReport.map(r => (<div key={r.id} className="app-card flex justify-between items-center border-l-8 border-slate-100 text-black py-4"><div className="flex-1"><p className="font-black text-sm text-black">{r.staff_name}</p><p className="text-[10px] text-slate-500">{r.work_date} (休憩:{r.break_time})</p><p className="text-xs font-bold text-[#75C9D7]">実働:{r.work_time}</p></div><div className="flex gap-1"><button onClick={() => handleEditClick(r)} className="p-2.5 bg-slate-50 text-slate-500 rounded-lg border-none cursor-pointer"><Edit size={14}/></button><button onClick={async () => { if(confirm("消去?")){ await supabase.from('breaks').delete().eq('timecard_id', r.id); await supabase.from('timecards').delete().eq('id', r.id); generateAdminReport(); }}} className="p-2.5 bg-red-50 text-red-400 rounded-lg border-none cursor-pointer"><Trash2 size={14}/></button></div></div>))}
            </div>
          )}
        </div>
      </main>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-[400] flex items-center justify-center p-4 backdrop-blur-sm text-black"><div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl text-black text-black text-black">
